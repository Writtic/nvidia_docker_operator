...
class NvidiaDockerHook(BaseHook, LoggingMixin):
    """
    Interact with a private Nvidia-Docker registry.
...
class NvidiaDockerOperator(BaseOperator):
    """
    Execute a command inside a nvidia-docker container for gpu processing.
    ...
    :type docker_conn_id: str
    :param gpu_devices: GPU device ID of environment 'NVIDIA_VISIBLE_DEVICES'
    :type gpu_devices: str
    """
    ...
    def __init__(
        ...
        super(NvidiaDockerOperator, self).__init__(*args, **kwargs)
    ...
    def execute(self, context):
        ...
        with TemporaryDirectory(prefix='airflowtmp') as host_tmp_dir:
            ...
            self.environment['NVIDIA_VISIBLE_DEVICES'] = self.gpu_devices
            self.volumes.append('{0}:{1}'.format(host_tmp_dir, self.tmp_dir))
            self.log.info(
                'environment [env: {}]'.format(self.environment))
            ...
            self.log.info(
                'Start docker container with [id: {}]'.format(self.container['Id']))
...
class NvidiaDockerPlugin(AirflowPlugin):
    name = "nvidia_docker_operator"
    operators = [NvidiaDockerOperator]
    # A list of class(es) derived from BaseHook
    hooks = [NvidiaDockerHook]
